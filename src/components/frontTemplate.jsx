    ...
    <div className='w-screen h-screen '>
      {
        item.map((item,index)=>{
            ...
            return(
            <div key={index} className={`flex ${alignment==="first"?"flex-row-reverse":"flex-row"} `}>
                ...
                <p className='py-12 text-xl font-normal font-poppins'>{item.desc}</p>
                </div>
                    
                </div>
                <div className='w-1/2 h-screen bg-center rounded-l-[50px] bg-cover bg-no-repeat  ' style={{backgroundImage:`url(${item.link})`}}>
