    ...
    <div className='h-screen w-full lg:mb-5 mb-10 items-center flex relative overflow-hidden'>
      ...
      <div className='z-10 lg:px-24 px-10 flex flex-col gap-14 w-full'>
        <div className='flex flex-col gap-6'>
          {defaultSubtitle && (
            <p className='lg:text-4xl text-sm font-poppins font-semibold text-white'>{defaultSubtitle}</p>
          )}
          {defaultTitle && (
            <p className='font-satisfy lg:text-9xl text-6xl font-normal text-white'>{defaultTitle}</p>
          )}
        </div>
        ...
        {onClick && (
          <p onClick={onClick} className='absolute bottom-10 text-white font-poppins font-semibold cursor-pointer left-[50%] translate-x-[-50%] flex items-center gap-2'>
