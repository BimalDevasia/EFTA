...
function navbar() {
    ...
    const pathname= usePathname()!='/' ? usePathname().split('/').filter(Boolean):usePathname() ;
  ...
  return (
    <div className='fixed flex px-36 w-screen justify-between items-center h-24 z-50 bg-white'>
        <Link href="/">
        <svg
        className={`w-24 h-24  ${selPage==="/"?"fill-primary_color":selPage==='/gifts'?(pathname.length===1?"fill-white":"fill-gift_blue"):""} `}
    viewBox="0 0 121 54"
    ...
    />
        ...
        <div className='flex items-center gap-14 '>
            {items.slice(selPage==='/'?0:1).map((item,index)=>(
                <div key={index} onClick={()=>setSelPage(item.path)} className={` relative text-2xl ${selPage===item.path?"border-x-2 border-t-2  rounded-t-2xl":""} ${selPage==="/"?"border-primary_color":selPage==='/gifts'?(pathname.length===1?"border-white":"border-gift_blue"):""} ${selPage==="/"?"text-primary_color":selPage==='/gifts'?(pathname.length===1?"text-white":"text-gift_blue"):""} font-semibold font-poppins px-5 py-3`}>
                  
                  <div></div>
                    <Link href={item.path}>{item.id}</Link>
                </div>
            ))}
            <PiShoppingCartSimpleFill className={`w-8 h-8 ${selPage==="/"?"text-primary_color":selPage==='/gifts'?(pathname.length===1?"text-white":"text-gift_blue"):""}  ${selPage==='/'?"hidden":""} `} />
        </div>
    </div>
  )
}
